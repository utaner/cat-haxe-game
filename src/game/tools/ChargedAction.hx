...
/** An utility class to manage BaseEntity.Entity charged actions, with a very low memory footprint (garbage collector friendly) **/
class ChargedAction implements dn.struct.RecyclablePool.Recyclable {
    ...
    public var remainS(get, never):Float;

    inline function get_remainS() return M.fclamp(durationS - elapsedS, 0, durationS);
