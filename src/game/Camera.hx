...
class Camera extends GameChildProcess {
    ...
    inline function get_cWid() return M.ceil(pxWid / Const.GRID);

    public var cHei(get, never):Int;

    inline function get_cHei() return M.ceil(pxHei / Const.GRID);
    ...
    inline function get_pxLeft() return Std.int(clampedFocus.levelX - pxWid * 0.5);

    public var pxRight(get, never):Int;

    inline function get_pxRight() return Std.int(pxLeft + (pxWid - 1));

    public var pxTop(get, never):Int;

    inline function get_pxTop() return Std.int(clampedFocus.levelY - pxHei * 0.5);

    public var pxBottom(get, never):Int;

    inline function get_pxBottom() return pxTop + pxHei - 1;

    /** Center X in pixels **/
    public var centerX(get, never):Int;

    inline function get_centerX() return Std.int((pxLeft + pxRight) * 0.5);

    /** Center Y in pixels **/
    public var centerY(get, never):Int;

    inline function get_centerY() return Std.int((pxTop + pxBottom) * 0.5);

    /** Camera bound coords in grid cells **/
    public var cLeft(get, never):Int;

    inline function get_cLeft() return Std.int(pxLeft / Const.GRID);

    public var cRight(get, never):Int;

    inline function get_cRight() return M.ceil(pxRight / Const.GRID);

    public var cTop(get, never):Int;

    inline function get_cTop() return Std.int(pxTop / Const.GRID);

    public var cBottom(get, never):Int;

    inline function get_cBottom() return M.ceil(pxBottom / Const.GRID);
	...
	**/
    ...
    /**
		Enable auto tracking on given BaseEntity.BaseEntity. If `immediate` is true, the camera is immediately positioned over the BaseEntity.Entity, otherwise it just moves to it.
	**/
    public function trackEntity(e:BaseEntity, immediate:Bool, speed = 1.0) {
        target = e;
